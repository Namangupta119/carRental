...
const Dashboard = () => {
  ...
  const dashboardCards=[
    ...
    {title: "Total Booking", value:data.totalBookings, icon: assets.carIconColored},
    {title: "Pending", value:data.pendinBbookings, icon: assets.carIconColored},
...
<div className='grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 my-8 max-w-3xl'>
  {dashboardCards.map((card,index)=>(
    <div key={index} className='flex gap-2 items-center justify-between p-4 rounded-md  border  border-borderColor'>
      <div>
        <h1 className='text-xs text-grey-500'>{card.title}</h1>
...
<div className='flex flex-wrap items-start gap-6 mb-8 w-full'>
  ...
  <div className='p-4 md:p-6 border border-borderColor rounded-md max-w-lg w-full'>
    ...
    <p className='text-green-500'>Latest Customer Bookings</p>
...
<div className='flex items-center gap-2 font-medium'>
  ...
  <p className='px-3 py-0.5 border border-borderColor rounded-full tetx-sm'>{booking.status}</p>
</div>
